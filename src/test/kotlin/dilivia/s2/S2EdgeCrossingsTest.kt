...
class S2EdgeCrossingsTest : S2GeometryTestCase() {
    ...
    fun printIntersectionStats() {
        ...
        for (i in S2EdgeCrossings.IntersectionMethod.values()) {
            ...
            println("%10s %9d %5.1f%% %9d %5.1f%%  %5.1f%%".format(
                    i.methodName,
    ...
    val kGetDistanceAbsError = S1Angle.radians(3 * DBL_EPSILON)

    fun testIntersectionError() {
        ...
        for (iter in 0 until 5000) {
            ...
            do {
                ...
                expected = S2EdgeCrossings.getIntersectionExact(a, b, c, d)
                    ...
                    || S2EdgeDistances.getDistance(expected, a, b) > S1Angle.radians(3 * DBL_EPSILON) + kGetDistanceAbsError
                    || S2EdgeDistances.getDistance(expected, c, d) > S1Angle.radians(3 * DBL_EPSILON) + kGetDistanceAbsError
            )

            // Each constructed edge should be at most 1.5 * DBL_EPSILON away from the
            // original point P.
            assertTrue(S2EdgeDistances.getDistance(p, a, b) <= S1Angle.radians(1.5 * DBL_EPSILON) + kGetDistanceAbsError)
            assertTrue(S2EdgeDistances.getDistance(p, c, d) <= S1Angle.radians(1.5 * DBL_EPSILON) + kGetDistanceAbsError)
            ...
            assertTrue(S2EdgeDistances.getDistance(expected, a, b) <= S1Angle.radians(3 * DBL_EPSILON) + kGetDistanceAbsError)
            assertTrue(S2EdgeDistances.getDistance(expected, c, d) <= S1Angle.radians(3 * DBL_EPSILON) + kGetDistanceAbsError)
            assertTrue(S1Angle(expected, p) <= S1Angle.radians(3 * DBL_EPSILON / slope) + S2EdgeCrossings.kIntersectionError)
            ...
            assertTrue(dist_ab <= S2EdgeCrossings.kIntersectionError + kGetDistanceAbsError)
            assertTrue(dist_cd <= S2EdgeCrossings.kIntersectionError + kGetDistanceAbsError)
            max_edge_dist = maxOf(max_edge_dist, maxOf(dist_ab, dist_cd))
            val point_dist = S1Angle(expected, actual)
            assertTrue(point_dist <= S2EdgeCrossings.kIntersectionError)
    ...
    fun testGrazingIntersections() {
        ...
        for (iter in 0 until 1000) {
            ...
            if (S1Angle(xcd, xce) > 2 * S2EdgeCrossings.kIntersectionError) {
                assertEquals(S2Predicates.sign(c, d, e) == S2Predicates.sign(c, a, b), S2Predicates.sign(ab, xcd, xce) > 0)
            }
        }
        printIntersectionStats()
    }

    fun testExactIntersectionUnderflow() {
        ...
        assertEquals(S2Point(1.0, 1e-300, 0.0), S2EdgeCrossings.getIntersection(a0, a1, b0, b1))
    }

    fun testGetIntersectionInvariants() {
        ...
        for (iter in 0 until kIters) {
            ...
            assertEquals((a - b).norm2(), (c - d).norm2())
            ...
            assertEquals(result, S2EdgeCrossings.getIntersection(a, b, c, d))
        }
    }
