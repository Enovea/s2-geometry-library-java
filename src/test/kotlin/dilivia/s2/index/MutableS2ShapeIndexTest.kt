...
import dilivia.s2.S2EdgeClipping.intersectsRect
import dilivia.s2.S2EdgeClipping.kIntersectsRectErrorUVDist
...
class MutableS2ShapeIndexTest : S2GeometryTestCase() {
    ...
    fun quadraticValidate() {
        ...
        while (true) {
            ...
            for (id in 0 until index.numShapeIds()) {
            ...
            assertTrue(short_edges <= index.options().maxEdgesPerCell)
            if (it.done()) break
            it.next()
        }
    }
    ...
    // the given cell id.
    fun validateEdge(a: S2Point, b: S2Point, id: S2CellId, indexhas_edge: Boolean) {
        ...
        val bound = id.getBoundUV().expanded(padding)
        val clippedEdge = S2EdgeClipping.clipToPaddedFace(a, b, id.face(), padding)
        assertEquals(clippedEdge != null && intersectsRect(clippedEdge.first, clippedEdge.second, bound), indexhas_edge)
    }
    ...
    fun validateInterior(shape: S2Shape?, id: S2CellId, indexcontains_center: Boolean) {
        if (shape == null) {
            assertFalse(indexcontains_center)
        } else {
            assertEquals(S2Shape.containsBruteForce(shape, id.toPoint()), indexcontains_center)
        }
    }

    fun testIteratorMethods(index: MutableS2ShapeIndex) {
        val it = index.cellIterator(InitialPosition.BEGIN)
        assertFalse(it.prev())
        it.finish()
        assertTrue(it.done())
        ...
        while (!it.done()) {
            ...
            for (skipped_id in skipped) {
                assertFalse(it2.locate(skipped_id.toPoint()))
                assertEquals(CellRelation.DISJOINT, it2.locate(skipped_id))
                it2.begin()
                it2.seek(skipped_id)
            ...
            if (!cellid.isFace()) {
                ...
                assertTrue(it2.id() <= cellid)
                assertTrue(it2.id() >= cellid.parent().rangeMin())
            }
            if (!cellid.isLeaf()) {
                for (i in 0..3) {
                    it2.begin()
    ...
    /*
      fun testSpaceUsed()
      {
          index.Add(make_unique<S2EdgeVectorShape>(S2Point(1, 0, 0),
                  S2Point(0, 1, 0)))
          assertFalse(index.is_fresh())
          ...
          assertTrue(index.is_fresh())
  
          assertTrue(size_after > size_before)
      }
  */
    fun testNoEdges() {
        val it = index.cellIterator(InitialPosition.BEGIN)
    ...
    fun testOneEdge() {
        assertEquals(0, index.add(S2EdgeVectorShape(S2Point(1, 0, 0), S2Point(0, 1, 0))))
        quadraticValidate()
        testIteratorMethods(index)
    }

    fun testShrinkToFitOptimization() {
    ...
    fun testLoopsSpanningThreeFaces() {
        fail("TODO")
//        S2Polygon polygon
...
//        TestEncodeDecode()
    ...
    fun testManyIdenticalEdges() {
        ...
        for (i in 0 until kNumEdges) {
            assertEquals(i, index.add(S2EdgeVectorShape(a, b)))
        }
        quadraticValidate()
        testIteratorMethods(index)
    ...
    fun testDegenerateEdge() {
        ...
        while (!it.done()) {
            ...
            assertEquals(1, it.cell().numClipped)
            assertEquals(1, it.cell().clipped(0).numEdges)
            it.next(); ++count
        }
        assertEquals(3, count)
    }

    fun testManyTinyEdges() {
...
TEST(MutableS2ShapeIndex, MixedGeometry) {
    ...
    assertEquals(S2ShapeIndex::DISJOINT, it.Locate(S2CellId::FromFace(1)))
}
