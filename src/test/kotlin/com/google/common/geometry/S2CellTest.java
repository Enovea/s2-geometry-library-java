...
public strictfp class S2CellTest extends GeometryTestCase {
    ...
    public void testFaces() {
        ...
        for (int face = 0; face < 6; ++face) {
            ...
            for (int k = 0; k < 4; ++k) {
                S2Point edgeRaw = cell.getEdgeRaw(k);
                System.out.println(edgeCounts.containsKey(edgeRaw) + " Edge raw: " + edgeRaw + " in: " + edgeCounts);
                if (edgeCounts.containsKey(edgeRaw)) {
                    edgeCounts.put(edgeRaw, edgeCounts.get(edgeRaw) + 1);
                } else {
                    edgeCounts.put(edgeRaw, 1);
                }
                ...
                assertDoubleNear(cell.getVertexRaw(k).dotProd(edgeRaw), 0);
    ...
    static void gatherStats(S2Cell cell) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            avgEdge += 0.25 * edge;
            S2Point mid = S2Point.plus(cell.getVertexRaw(i), cell
                    .getVertexRaw((i + 1) & 3));
            ...
            if (i < 2) {
                ...
                double angleSpan = cell.getEdgeRaw(i).angle(
                        S2Point.unaryMinus(cell.getEdgeRaw(i ^ 2)));
    ...
    public void testSubdivide(S2Cell cell) {
        ...
        for (int i = 0; i < 4; ++i, childId = childId.next()) {
            ...
            S2Point edge = S2Point.plus(center, S2Projections.getUAxis(children[i].face()));
            S2Point corner = S2Point.plus(edge, S2Projections.getVAxis(children[i].face()));
            for (int j = 0; j < 4; ++j) {
