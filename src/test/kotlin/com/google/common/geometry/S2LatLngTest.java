...
public strictfp class S2LatLngTest extends GeometryTestCase {
  ...
  public void testBasic() {
    ...
    assertTrue(llRad.lat().getRadians() == S2.M_PI_4);
    assertTrue(llRad.lng().getRadians() == S2.M_PI_2);
    ...
    assertDoubleNear(better.lng().getRadians(), S1Angle.degrees(-160).getRadians());
    ...
    assertTrue(better.isValid());
    assertEquals(better.lat(), S1Angle.degrees(-90));
    assertDoubleNear(better.lng().getRadians(), 0);
  ...
  public void testConversion() {
    ...
    assertEquals(
        new S2LatLng(S2LatLng.fromRadians(-S2.M_PI_2, 1).toPoint()).lat().getRadians(), -S2.M_PI_2);
    assertDoubleNear(
        Math.abs(new S2LatLng(S2LatLng.fromDegrees(12.2, 180.0).toPoint()).lng().degrees()), 180.0);
    assertEquals(
        Math.abs(new S2LatLng(S2LatLng.fromRadians(0.1, -S2.M_PI).toPoint()).lng().getRadians()),
        S2.M_PI);
  ...
  public void testDistance() {
    assertEquals(
        S2LatLng.fromDegrees(90, 0).getDistance(S2LatLng.fromDegrees(90, 0)).getRadians(), 0.0);
    assertDoubleNear(
