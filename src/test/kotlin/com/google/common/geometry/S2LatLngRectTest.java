...
public strictfp class S2LatLngRectTest extends GeometryTestCase {
    ...
    public void testBasic() {
        ...
        testCellOps(rectFromDegrees(-41, 160, -40, -160), S2Cell.fromFacePosLevel(5, (byte) 0, 0), 1);
        {
            ...
            S2LatLng v0 = S2LatLng.fromPoint(cell0tr.getVertexRaw(0));
            testCellOps(
    ...
    public void testEdgeBound() {
        ...
        assertDoubleNear(getEdgeBound(1, 1, 1, 1, -1, 1).lat().getHi(), S2.M_PI_4); // Max,
        // CW
        assertDoubleNear(getEdgeBound(1, -1, 1, 1, 1, 1).lat().getHi(), S2.M_PI_4); // Max,
        // CCW
        assertDoubleNear(getEdgeBound(1, -1, -1, -1, -1, -1).lat().getLo(), -S2.M_PI_4); // Min,
        // CW
        assertDoubleNear(getEdgeBound(-1, 1, -1, -1, -1, -1).lat().getLo(), -S2.M_PI_4); // Min,
        // CCW
    ...
    public void testGetDistanceRandomPairs() {
        ...
        for (int i = 0; i < 10000; ++i) {
            S2LatLngRect a = S2LatLngRect.fromPointPair(S2LatLng.fromPoint(randomPoint()), S2LatLng.fromPoint(randomPoint()));
            S2LatLngRect b = S2LatLngRect.fromPointPair(S2LatLng.fromPoint(randomPoint()), S2LatLng.fromPoint(randomPoint()));
            verifyGetDistance(a, b);


            S2LatLng c = S2LatLng.fromPoint(randomPoint());
    ...
    private static S1Angle bruteForceDistance(S2LatLngRect a, S2LatLngRect b) {
        ...
        S2LatLng pntA[] =
                {S2LatLng.fromLatLng(a.latLo(), a.lngLo()), S2LatLng.fromLatLng(a.latLo(), a.lngHi()),
                        S2LatLng.fromLatLng(a.latHi(), a.lngHi()), S2LatLng.fromLatLng(a.latHi(), a.lngLo())};
        S2LatLng pntB[] =
                {S2LatLng.fromLatLng(b.latLo(), b.lngLo()), S2LatLng.fromLatLng(b.latLo(), b.lngHi()),
                        S2LatLng.fromLatLng(b.latHi(), b.lngHi()), S2LatLng.fromLatLng(b.latHi(), b.lngLo())};
    ...
    private static S1Angle bruteForceRectPointDistance(S2LatLngRect a, S2LatLng b) {
        ...
        S1Angle bToLoLng = S2EdgeUtil.getDistance(
                b.toPoint(),
                S2LatLng.fromLatLng(a.latLo(), a.lngLo()).toPoint(),
                S2LatLng.fromLatLng(a.latHi(), a.lngLo()).toPoint());
        S1Angle bToHiLng = S2EdgeUtil.getDistance(
                b.toPoint(),
                S2LatLng.fromLatLng(a.latLo(), a.lngHi()).toPoint(),
                S2LatLng.fromLatLng(a.latHi(), a.lngHi()).toPoint()
        );
    ...
    private static S1Angle getDistance(S2LatLng x, S1Angle lat, S1Interval interval) {
        ...
        return S1Angle.min(
                x.getDistance(S2LatLng.fromLatLng(lat, S1Angle.radians(interval.getLo()))),
                x.getDistance(S2LatLng.fromLatLng(lat, S1Angle.radians(interval.getHi())))
        );
    }
