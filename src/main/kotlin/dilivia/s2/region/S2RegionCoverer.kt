...
class S2RegionCoverer(
    ...
    fun getFastCovering(region: S2Region, covering: MutableList<S2CellId>) {
        ...
        logger.debug { "getFastCovering(region = $region) = $covering" }
    }
    ...
    private fun addCandidate(candidate: Candidate?) {
        ...
        assertEQ(0, candidate.numChildren)
    ...
    private fun getCoveringInternal(region: S2Region) {
        ...
        assert({ isCanonical(result) }, { "getCoveringInternal(region = $region) is not canonical: $result" })
    }
