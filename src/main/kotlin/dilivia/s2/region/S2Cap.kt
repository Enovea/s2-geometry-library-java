...
class S2Cap(val center: S2Point, val radius: S1ChordAngle) : S2Region {
    ...
    fun addCap(other: S2Cap): S2Cap {
        return if (isEmpty) {
            ...
            val newRadius = maxOf(radius, dist.plusError(DBL_EPSILON * dist.length2))
    ...
    fun expanded(distance: S1Angle): S2Cap {
        assertGreaterOrEquals(distance.radians, 0.0)
    ...
    fun approxEquals(other: S2Cap, maxErrorAngle: S1Angle = S1Angle.radians(1e-14)): Boolean {
        ...
        return (S2Point.approxEquals(center, other.center, maxErrorAngle) && abs(r2 - otherR2) <= maxError)
        ...
        get() {
            ...
            if (!allLongitudes) {
                ...
                if (sinA <= sinC) {
                    ...
                    lng[0] = (centerLl.lng().radians - angleA).IEEErem(2 * M_PI)
                    lng[1] = (centerLl.lng().radians + angleA).IEEErem(2 * M_PI)
                }
            }
            return S2LatLngRect(
    ...
    companion object {
        ...
        fun fromCenterHeight(center: S2Point, height: Double): S2Cap {
            assertPointIsUnitLength(center)
