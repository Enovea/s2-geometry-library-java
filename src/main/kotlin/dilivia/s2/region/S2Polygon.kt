...
class S2Polygon() : S2Region {
    ...
    constructor(loops: List<S2Loop>, check: Boolean = true): this() {
        initNested(loops, check)
    }

    // Convenience constructor that creates a polygon with a single loop
    // corresponding to the given cell.
    constructor(cell: S2Cell, check: Boolean = true): this() {
        init(S2Loop(cell), check)
    }
    ...
    fun initNested(loops: List<S2Loop>, check: Boolean = true) {
        clearLoops()
        ...
        for (i in 0 until numLoops()) {
            insertLoop(loop(i), null, loopMap)
        }
    ...
    // inverted.)
    fun initOriented(loops: List<S2Loop>, check: Boolean = true) {
        ...
        this.loops.indices.forEach { i ->
            ...
            if ((containedOrigin.contains(loop(i)) != containsOrigin) != loop(i).isHole()) {
                ...
                if (check) {
                    ...
                    assertTrue(isValid())  // Always fails.
                }
            }
        }
    }
    ...
    fun init(loop: S2Loop, check: Boolean = true) {
    ...
    fun copy(src: S2Polygon, check: Boolean = true) {
        clearLoops()
        ...
        initIndex(check);  // TODO(ericv): Copy the index efficiently.
    }
    ...
    fun getArea(): Double = loops.map { l -> l.sign() * l.getArea() }.sum()
    ...
    fun getCentroid(): S2Point =
            loops.map { l -> l.getCentroid() * l.sign().toDouble() }.fold(S2Point()) { acc, point -> acc + point }
        ...
        if (isEmpty()) {
            ...
            for (i in 1 until numLoops()) {
                if (loop(i).depth == 0) {
                    ...
                    if (bestAngle == kNone) bestAngle = loop(best).getCurvature()
                    val angle = loop(i).getCurvature()
            ...
            assertEQ(newLoops.size, numLoops())
        }
        clearIndex()
        initLoopProperties()
    }
...
//        return initToOperation(S2BooleanOperation.OpType.SYMMETRIC_DIFFERENCE, snap_function, a, b)
    ...
    fun initToApproxIntersection(a: S2Polygon, b: S2Polygon, snap_radius: S1Angle) {
        initToIntersection(a, b, IdentitySnapFunction(snap_radius))
    }

    fun initToApproxUnion(a: S2Polygon, b: S2Polygon, snap_radius: S1Angle) {
        initToUnion(a, b, IdentitySnapFunction(snap_radius))
    }

    fun initToApproxDifference(a: S2Polygon, b: S2Polygon, snap_radius: S1Angle) {
        initToDifference(a, b, IdentitySnapFunction(snap_radius))
    }

    fun initToApproxSymmetricDifference(a: S2Polygon, b: S2Polygon, snap_radius: S1Angle) {
...
//        return result;
    ...
    private fun getCellEdgeIncidenceMask(cell: S2Cell, p: S2Point, tolerance_uv: Double, check: Boolean = true): UByte {
        var mask: UByte = 0.toUByte()
        val uv = S2Coords.faceXYZtoUV(cell.face(), p)
        if (uv != null) {
            val bound = cell.boundUV()
            if (check) Assertions.assert { bound.expanded(tolerance_uv).contains(uv) }
...
//        }
    ...
    fun initToCellUnionBorder(cells: S2CellUnion) {
        ...
        // different edges.
        val snap_radius = 0.5 * S2CellMetrics.kMinWidth.getValue(S2CellId.kMaxLevel)
        val builder = S2Builder(S2Builder.Options(
...
//        return MakeS2ContainsPointQuery(index).contains(p)
    ...
    class Shape(val polygon: S2Polygon) : S2Shape() {
        ...
        init {
            ...
            if (!polygon.isFull()) {
                ...
                val num_loops = polygon.numLoops()
                if (num_loops > kMaxLinearSearchLoops) {
                    cumulativeEdges = IntArray(num_loops)
                }
                else {
        ...
        override fun getReferencePoint(): ReferencePoint {
            ...
            return ReferencePoint(S2Point.origin(), containsOrigin)
        }
        ...
        override fun chain(chain_id: Int): Chain {
            assertLT(chain_id, numChains)
            if (cumulativeEdges.isNotEmpty()) {
                return Chain(cumulativeEdges[chain_id], polygon.loop(chain_id).numVertices())
            } else {
                var e = 0
                for (j in 0 until chain_id) e += polygon.loop(j).numVertices()
                ...
                val numVertices = polygon.loop(chain_id).numVertices()
                return Chain(e, if(numVertices == 1) 0 else numVertices)
            }
        }

        override fun chainEdge(chainId: Int, offset: Int): Edge {
            assertLT(chainId, numChains)
            assertLT(offset, polygon.loop(chainId).numVertices())
    ...
    private fun initOneLoop(check: Boolean = true) {
        assertEQ(1, numLoops())
        val loop = loops[0]
        loop.depth = 0
    ...
    // Compute num_vertices_, bound_, subregion_bound_.
    private fun initLoopProperties(check: Boolean = true) {
        numVertices = 0
    ...
    private fun insertLoop(new_loop: S2Loop, parent: S2Loop?, loop_map: LoopMap) {
        var parent = parent
    ...
    fun initLoops(loop_map: LoopMap) {
        val loopStack: Deque<S2Loop> = LinkedList<S2Loop>()
        var depth = -1
    ...
    private fun initIndex(check: Boolean = true) {
        assertEQ(0, index.numShapeIds())
        index.add(Shape(this));
