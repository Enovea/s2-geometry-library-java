...
class S2Cell : S2Region, Comparable<S2Cell> {
    ...
    fun getMaxDistance(a: S2Point, b: S2Point): S1ChordAngle {
        ...
        val max_dist = maxOf(getMaxDistance(a), getMaxDistance(b))
    ...
    override val rectBound: S2LatLngRect
        get() {
            if (level > 0) {
                ...
                val i = if (S2Coords.getUAxis(face())[2] == 0.0) {
                    if (u < 0) 1 else 0
                } else {
                    if (u > 0) 1 else 0
                }
                val j = if (S2Coords.getVAxis(face())[2] == 0.0) {
                    if (v < 0) 1 else 0
                } else {
                    if (v > 0) 1 else 0
                }
            ...
            assertEQ((if (face() < 3) 1.0 else -1.0), S2Coords.getNorm(face())[face() % 3])
            ...
            return bound.expanded(S2LatLng.fromRadians(DBL_EPSILON, 0.0))
        }
    ...
    override operator fun contains(p: S2Point): Boolean {
        ...
        return boundUV.expanded(DBL_EPSILON).contains(uv)
    }
    ...
    companion object {
        ...
        fun averageArea(level: Int): Double {
            return S2CellMetrics.kAvgArea.getValue(level)
        }
