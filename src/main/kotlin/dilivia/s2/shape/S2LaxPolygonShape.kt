...
class S2LaxPolygonShape : S2Shape {
    ...
    private var cumulativeVertices: IntArray = IntArray(0)
    ...
    override fun chain(chain_id: Int): Chain {
        assertLT(chain_id, numLoops)
        return if (numLoops == 1) {
            Chain(0, numVertices())
        } else {
            val start = cumulativeVertices[chain_id]
            Chain(start, cumulativeVertices[chain_id + 1] - start)
        }
    }

    override fun chainEdge(chainId: Int, offset: Int): Edge {
        assertLT(chainId, numLoops)
        assertLT(offset, numLoopVertices(chainId))
    ...
    override fun chainPosition(edgeId: Int): ChainPosition {
        ...
        if (numLoops == 1) {
            ...
            return ChainPosition(cumulativeVertices[nextIdx] - (cumulativeVertices[1]), edgeId - cumulativeVertices[nextIdx-1])
        }
    }

    override val typeTag: TypeTag = TypeTags.kLaxPolygonTypeTag

    private fun initSpans(loops: List<S2Loop.S2PointLoopSpan>) {
        numLoops = loops.size
        if (numLoops == 0) {
            vertices = emptyArray()
