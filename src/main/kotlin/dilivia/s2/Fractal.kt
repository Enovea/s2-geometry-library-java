...
class Fractal(
    ...
    fun setDimension(dimension: Double) {
        assertGE(dimension, 1.0)
        assertLT(dimension, 2.0)
    ...
    fun setLevelForApproxMinEdges(minEdges: Int) {
        ...
        setMinLevel((0.5 * log2(minEdges / 3.0)).roundToInt())
    }

    fun setLevelForApproxMaxEdges(maxEdges: Int) {
        // Map values in the range [3*(4**n)/2, 3*(4**n)*2) to level n.
        setMaxLevel((0.5 * log2(maxEdges / 3.0)).roundToInt())
    }
    ...
    fun makeLoop(frame: Matrix3x3, nominalRadius: S1Angle): S2Loop {
    ...
    private fun computeOffsets() {
        edgeFraction = 4.0.pow(-1.0 / dimension)
    ...
    private fun getR2Vertices(vertices: MutableList<R2Point>) {
        ...
        val v0 = R2Vector(1.0, 0.0)
        val v1 = R2Vector(-0.5, sqrt(3.0)/2)
        val v2 = R2Vector(-0.5, -sqrt(3.0)/2)
