...
class S2LatLngRect(val lat: R1Interval, val lng: S1Interval) : S2Region {
    ...
    fun addPoint(ll: S2LatLng): S2LatLngRect {
        assertLatLngIsValid(ll)
        val newLat = lat.addPoint(ll.lat().radians)
        val newLng = lng.addPoint(ll.lng().radians)
    ...
    override val capBound: S2Cap
        get() {
            ...
            if (lngSpan.IEEErem(2 * M_PI) >= 0 && lngSpan < 2 * M_PI_2) {
    ...
    companion object {
        ...
        fun fromPointPair(p1: S2LatLng, p2: S2LatLng): S2LatLngRect {
            assertLatLngIsValid(p1)
            assertLatLngIsValid(p2)
            return S2LatLngRect(
         ...
         */
        @JvmField
        val empty: S2LatLngRect = S2LatLngRect(R1Interval.empty(), S1Interval.empty)
         ...
         */
        @JvmField
        val full: S2LatLngRect = S2LatLngRect(fullLat(), fullLng())
        ...
        @JvmStatic
        fun getDirectedHausdorffDistance(lng_diff: Double, a: R1Interval, b: R1Interval): S1Angle {
            ...
            val b_lng = lng_diff
            // Two endpoints of b.
            val b_lo = S2LatLng.fromRadians(b.lo, b_lng).toPoint()
            val b_hi = S2LatLng.fromRadians(b.hi, b_lng).toPoint()

            // Handling of each case outlined at the top of the function starts here.
            // This is initialized a few lines below.
            var max_distance: S1Angle = S1Angle.zero
            ...
            if (lng_diff <= M_PI_2) {
                ...
                if (a.contains(0.0) && b.contains(0.0)) {
                    max_distance = maxOf(max_distance, S1Angle.radians(lng_diff))
                }
            } else {
                // Case B2.
                val p = getBisectorIntersection(b, b_lng)
                val p_lat = S2LatLng.latitude(p).radians
                if (a.contains(p_lat)) {
                    max_distance = maxOf(max_distance, S1Angle(p, b_lo))
                }

                // Case B3.
                if (p_lat > a.lo) {
                    max_distance = maxOf(max_distance, getInteriorMaxDistance(R1Interval(a.lo, min(p_lat, a.hi)), b_lo))
                }
                if (p_lat < a.hi) {
                    max_distance = maxOf(max_distance, getInteriorMaxDistance(R1Interval(max(p_lat, a.lo), a.hi), b_hi))
                }
            }
