...
class S2LatLngRectBounder {
    ...
    fun addPoint(point: S2Point) {
        Assertions.assert { S2Point.isUnitLength(point) }
    ...
    fun addLatLng(b_latlng: S2LatLng) = addInternal(b_latlng.toPoint(), b_latlng)
    ...
    fun getBound(): S2LatLngRect {
        ...
        val kExpansion = S2LatLng.fromRadians(2 * DBL_EPSILON, 0.0);
    ...
    private fun addInternal(b: S2Point, b_latlng: S2LatLng) {
        // Simple consistency check to verify that b and b_latlng are alternate
        // representations of the same vertex.
        Assertions.assert { S2Point.approxEquals(b, b_latlng.toPoint()) }

        if (bound.isEmpty) {
            bound = bound.addPoint(b_latlng)
        } else {
            ...
            if (n_norm < 1.91346e-15) {
                ...
                if (a.dotProd(b) < 0) {
                    ...
                    bound = bound.union(S2LatLngRect.fromPointPair(a_latlng, b_latlng))
                }
            } else {
                // Compute the longitude range spanned by AB.
                var lng_ab = S1Interval.fromPointPair(a_latlng.lng().radians, b_latlng.lng().radians)
                if (lng_ab.length >= M_PI - 2 * DBL_EPSILON) {
                ...
                val lat_ab = R1Interval.fromPointPair(a_latlng.lat().radians, b_latlng.lat().radians).toMutable()
                ...
                val m_error = 6.06638e-16 * n_norm + 6.83174e-31;
                if (m_a * m_b < 0 || abs(m_a) <= m_error || abs(m_b) <= m_error) {
                    ...
                    val max_lat = min(atan2(sqrt(n[0]*n[0] + n[1]*n[1]), abs(n[2])) + 3 * DBL_EPSILON, M_PI_2)
                    ...
                    val lat_budget = 2 * asin(0.5 * (a - b).norm() * sin(max_lat))
                    val max_delta = 0.5*(lat_budget - lat_ab.length) + DBL_EPSILON
                    ...
                    if (m_a <= m_error && m_b >= -m_error) {
                        lat_ab.hi = min(max_lat, lat_ab.hi + max_delta)
                    }
                    if (m_b <= m_error && m_a >= -m_error) {
                        lat_ab.lo = max(-max_lat, lat_ab.lo - max_delta)
                    }
                }
                bound = bound.union(S2LatLngRect(lat_ab, lng_ab))
            }
        }
        a = b;
        a_latlng = b_latlng;
    ...
    companion object {
        ...
        fun expandForSubregions(bound: S2LatLngRect): S2LatLngRect {
            ...
            val lng_gap = max(0.0, M_PI - bound.lng.length - 2.5 * DBL_EPSILON)
            ...
            val lat_expansion = 9 * DBL_EPSILON
            val lng_expansion = if(lng_gap <= 0) M_PI else 0.0
