...
class S2CellIdSnapFunction(level: Int = S2CellId.kMaxLevel) : SnapFunction() {
    ...
    var level: Int = level
        set(value) {
            assertGE(value, 0)
            assertLE(value, S2CellId.kMaxLevel)
            field = value
    ...
    override var snapRadius: S1Angle = minSnapRadiusForLevel(level)
        set(value) {
            assertGE(value, minSnapRadiusForLevel(level))
            assertLE(value, kMaxSnapRadius)
            field = value
        }
    ...
    override fun minVertexSeparation(): S1Angle {
        ...
        //    when snapped.
        val minEdge = S1Angle.radians(S2CellMetrics.kMinEdge.getValue(level))
        val maxDiag = S1Angle.radians(S2CellMetrics.kMaxDiag.getValue(level))
    ...
    override fun minEdgeVertexSeparation(): S1Angle {
        ...
        val minDiag = S1Angle.radians(S2CellMetrics.kMinDiag.getValue(level))
    ...
    companion object {
        ...
        fun minSnapRadiusForLevel(level: Int): S1Angle {
            ...
            return S1Angle.radians(0.5 * S2CellMetrics.kMaxDiag.getValue(level) + 4 * DBL_EPSILON)
        }
        ...
        fun levelForMaxSnapRadius(snapRadius: S1Angle): Int {
            ...
            return S2CellMetrics.kMaxDiag.getLevelForMaxValue(2.0 * (snapRadius.radians - 4 * DBL_EPSILON))
        }
