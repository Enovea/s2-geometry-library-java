...
import com.google.common.geometry.S2.M_SQRT1_2
...
class IntLatLngSnapFunction(exponent: Int = 7) : SnapFunction() {
    ...
    var exponent: Int = exponent
        set(value) {
            assertGE(value, kMinExponent)
            assertLE(value, kMaxExponent)
            field = value
    ...
    override var snapRadius: S1Angle = minSnapRadiusForExponent(exponent)
        set(value) {
            assertGE(value, minSnapRadiusForExponent(exponent))
    ...
    override fun snapPoint(point: S2Point): S2Point {
        assertGE(exponent, 0)  // Make sure the snap function was initialized.
        val input = S2LatLng.fromPoint(point)
        val lat = (input.lat().degrees() * fromDegrees).roundToInt()
        val lng = (input.lng().degrees() * fromDegrees).roundToInt()
    ...
    companion object {
        ...
        fun exponentForMaxSnapRadius(snapRadius: S1Angle): Int {
            ...
            radius -= S1Angle.radians((9 * M_SQRT2 + 1.5) * DBL_EPSILON)
            ...
            return max(kMinExponent,
                    min(kMaxExponent, ceil(exponent - 2 * DBL_EPSILON).toInt()))
        }
