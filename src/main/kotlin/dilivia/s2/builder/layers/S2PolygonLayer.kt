...
import dilivia.s2.builder.graph.get
...
class S2PolygonLayer(
    ...
    override fun build(g: Graph, error: S2Error) {
        if (labelSetIds != null) labelSetIds.clear()
        ...
        val loop_map = mutableMapOf<S2Loop, Pair<Int, Boolean>>()
        if (g.numEdges() == 0) {
            // The polygon is either full or empty.
            if (g.isFullPolygon(error)) {
                polygon.init(S2Loop(S2Loop.kFull))
            } else {
                polygon.initNested(mutableListOf<S2Loop>())
            }
        } else if (g.options.edge_type == EdgeType.DIRECTED) {
            val edge_loops = mutableListOf<EdgeLoop>()
            if (!g.getDirectedLoops(Graph.LoopType.SIMPLE, edge_loops, error)) {
                return;
            }
    ...
    private fun appendS2Loops(g: Graph, edge_loops: List<EdgeLoop>, loops: MutableList<S2Loop>) {
        ...
        for (edge_loop in edge_loops) {
            ...
            loops.add(S2Loop(vertices.toList()))
            vertices.clear()
        }
    }
