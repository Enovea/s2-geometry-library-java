...
class S2PolylineVectorLayer(
    ...
    init {
        assertEQ(labelSetIds == null, labelSetLexicon == null)
    }
    ...
    override fun build(g: Graph, error: S2Error) {
        ...
        for (edge_polyline in edgePolylines) {
            ...
            val polyline = S2Polyline(vertices, options.debug == S2Debug.ALLOW)
            vertices.clear()
            ...
            if (labelSetIds != null && labelSetLexicon != null) {
                ...
                val polylineLabels = mutableListOf<LabelSetId>()
                if (polylineLabels is ArrayList) {
                    polylineLabels.ensureCapacity(edge_polyline.size)
                }
                for (e in edge_polyline) {
                    fetcher.fetch(e, labels)
                    polylineLabels.add(labelSetLexicon.add(labels))
                }
                labelSetIds.addAll(polylineLabels)
            }
        }
    }
