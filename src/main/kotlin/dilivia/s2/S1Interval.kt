 ...
 * @constructor Internal constructor that assumes that both arguments are in the correct range, i.e. normalization from
 ...
 */
@Strictfp
open class S1Interval private constructor(lo: Double, hi: Double, checked: Boolean) : Cloneable {
    ...
    /**
     * Check if this interval contains all points of another one.
     * Works for empty, full, and singleton intervals.
     *
     * @param y An interval.
     ...
     * @return A new S1Interval that contains this interval and the point p.
     */
    open fun addPoint(p: Double): S1Interval {
        require(abs(p) <= M_PI)
        var cp = p
        if (cp == -M_PI) {
            cp = M_PI
    ...
    fun expanded(margin: Double): S1Interval {
        if (margin >= 0) {
            ...
            if (length + 2 * margin + 2 * DBL_EPSILON >= 2 * M_PI) return full
        } else {
            if (isFull) return this
            // Check whether this interval will be empty after expansion, allowing
            // for a 1-bit rounding error when computing each endpoint.
            if (length + 2 * margin - 2 * DBL_EPSILON <= 0) return empty
        }
        var result = S1Interval((lo - margin).IEEErem(2 * M_PI), (hi + margin).IEEErem(2 * M_PI))
    ...
    fun approxEquals(y: S1Interval, maxError: Double = 1e-15): Boolean {
        return when {
            ...
            else -> abs((y.lo - lo).IEEErem(2 * M_PI)) <= maxError
                    && abs((y.hi - hi).IEEErem(2 * M_PI)) <= maxError
    ...
    companion object {
        ...
        /** The empty interval.*/
        @JvmStatic
        val empty: S1Interval = S1Interval(M_PI, -M_PI, true)

        /** the full interval. */
        @JvmStatic
        val full: S1Interval = S1Interval(-M_PI, M_PI, true)
