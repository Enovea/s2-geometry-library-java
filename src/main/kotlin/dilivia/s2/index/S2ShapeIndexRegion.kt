...
class S2ShapeIndexRegion<T : S2ShapeIndex>(val index: T) : S2Region {
    ...
    private val contains_query: S2ContainsPointQuery<T> = S2ContainsPointQuery<T>(index)
    ...
    private val iter = contains_query.mutableIter()
    ...
    override fun contains(target: S2Cell): Boolean {
        val relation = iter.locate(target.id())
        ...
        for (s in 0 until cell.numClipped) {
            ...
            if (iter.id() == target.id()) {
                ...
                if (shape.dimension == 2 && !anyEdgeIntersects(clipped, target) && contains_query.shapeContains(iter, clipped, target.getCenter())) {
                    return true
                }
            }
        }
    ...
    companion object {
        ...
        private fun coverRange(first: S2CellId, last: S2CellId, cell_ids: MutableList<S2CellId>) {
            if (first == last) {
