...
import dilivia.s2.FaceSegmentVector
import dilivia.s2.R2Rect
import dilivia.s2.S2CellId
import dilivia.s2.S2EdgeClipping
import dilivia.s2.S2EdgeClipping.kFaceClipErrorUVCoord
...
class S2CrossingEdgeQuery(val index: S2ShapeIndex) {
    ...
    private val tmp_candidates: MutableSet<ShapeEdgeId> = mutableSetOf()
    ...
    fun getCrossingEdges(a0: S2Point, a1: S2Point, shape: S2Shape, type: CrossingType, edges: MutableList<ShapeEdge>): Unit {
        edges.clear()
    ...
    fun getCandidates(a0: S2Point, a1: S2Point): Set<ShapeEdgeId> {
        val edges = mutableSetOf<ShapeEdgeId>()
    ...
    fun getCandidates(a0: S2Point, a1: S2Point, shape: S2Shape): Set<ShapeEdgeId> {
        val edges = mutableSetOf<ShapeEdgeId>()
    ...
    // These versions can be more efficient when they are called many times,
    // since they do not require allocating a new vector on each call.
    fun getCandidates(a0: S2Point, a1: S2Point, edges: MutableSet<ShapeEdgeId>) {
        edges.clear()
    ...
    fun getCandidates(a0: S2Point, a1: S2Point, shape: S2Shape, edges: MutableSet<ShapeEdgeId>) {
        edges.clear()
    ...
    fun visitRawCandidates(a0: S2Point, a1: S2Point, visitor: ShapeEdgeIdVisitor): Boolean {
        ...
        if (num_edges <= kMaxBruteForceEdges) {
            val num_shape_ids = index.numShapeIds()
    ...
    fun visitCells(a0: S2Point, a1: S2Point, visitor: CellVisitor): Boolean {
        val segments: FaceSegmentVector = mutableListOf()
    ...
    fun visitCells(a0: S2Point, a1: S2Point, root: S2PaddedCell, visitor: CellVisitor): Boolean {
        Assertions.assertEQ(root.padding, 0.0)
    ...
    private fun visitCells(pcell: S2PaddedCell, edge_bound: R2Rect, visitor: CellVisitor): Boolean {
        ...
        Assertions.assertEQ(pcell.padding, 0.0)
        ...
        if (edge_bound[0].hi < center[0]) {
            ...
            return clipVAxis(edge_bound, center[1], 0, pcell, visitor);
        } else if (edge_bound[0].lo >= center[0]) {
            // Edge is entirely contained in the two right children.
            return clipVAxis(edge_bound, center[1], 1, pcell, visitor);
        } else {
            val child_bounds = Array<R2Rect>(2) { R2Rect.empty() }
            splitUBound(edge_bound, center[0], child_bounds);
            if (edge_bound[1].hi < center[1]) {
                // Edge is entirely contained in the two lower children.
                return (visitCells(S2PaddedCell(pcell, 0, 0), child_bounds[0], visitor) &&
                        visitCells(S2PaddedCell(pcell, 1, 0), child_bounds[1], visitor))
            } else if (edge_bound[1].lo >= center[1]) {
                // Edge is entirely contained in the two upper children.
                return (visitCells(S2PaddedCell(pcell, 0, 1), child_bounds[0], visitor) &&
                        visitCells(S2PaddedCell(pcell, 1, 1), child_bounds[1], visitor))
            } else {
                // The edge bound spans all four children.  The edge itself intersects
                // at most three children (since no padding is being used).
                return (clipVAxis(child_bounds[0], center[1], 0, pcell, visitor) &&
                        clipVAxis(child_bounds[1], center[1], 1, pcell, visitor));
            }
        }
    }
    ...
    private fun clipVAxis(edge_bound: R2Rect, center: Double, i: Int, pcell: S2PaddedCell, visitor: CellVisitor): Boolean {
        if (edge_bound[1].hi < center) {
            // Edge is entirely contained in the lower child.
            return visitCells(S2PaddedCell(pcell, i, 0), edge_bound, visitor);
        } else if (edge_bound[1].lo >= center) {
            // Edge is entirely contained in the upper child.
            return visitCells(S2PaddedCell(pcell, i, 1), edge_bound, visitor);
        } else {
            // The edge intersects both children.
            val child_bounds = Array<R2Rect>(2) { R2Rect.empty() }
            splitVBound(edge_bound, center, child_bounds);
            return (visitCells(S2PaddedCell(pcell, i, 0), child_bounds[0], visitor) &&
                    visitCells(S2PaddedCell(pcell, i, 1), child_bounds[1], visitor));
        }
    }

    // Split the current edge into two child edges at the given u-value "u" and
    // return the bound for each child.
    fun splitUBound(edge_bound: R2Rect, u: Double, child_bounds: Array<R2Rect>) {
        // See comments in MutableS2ShapeIndex::ClipUBound.
        val v = edge_bound[1].project(S2EdgeClipping.interpolateDouble(u, a0_[0], a1_[0], a0_[1], a1_[1]))
    ...
    companion object {
        ...
        fun splitBound(edge_bound: R2Rect, u_end: Int, u: Double, v_end: Int, v: Double, child_bounds: Array<R2Rect>) {
            val childBound0 = edge_bound.toMutable()
            childBound0[0][1 - u_end] = u;
            childBound0[1][1 - v_end] = v;
            child_bounds[0] = childBound0
            Assertions.assert { !child_bounds[0].isEmpty }
            Assertions.assert { edge_bound.contains(child_bounds[0]) }

            val childBound1 = edge_bound.toMutable()
            childBound1[0][u_end] = u;
            childBound1[1][v_end] = v;
            child_bounds[1] = childBound1;
            Assertions.assert { !child_bounds[1].isEmpty }
            Assertions.assert { edge_bound.contains(child_bounds[1]) }
        }
    }
