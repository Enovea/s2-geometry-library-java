...
class S2ClosestPointQueryBase<T: Distance<T>, D : Comparable<D>> {
    ...
    private val resultSet: PriorityQueue<Result<T, D>> = PriorityQueue(16) { d1, d2 -> d2.compareTo(d1)}

    private var queue: Deque<QueueEntry<T>> = ArrayDeque(16)

    // Temporaries, defined here to avoid multiple allocations / initializations.
    private lateinit var iter: S2PointIndexIterator<D>
    ...
    fun init(index: S2PointIndex<D>) {
        this.index = index
        this.iter = S2PointIndexIterator(index)
        reInit()
    }
    ...
    fun findClosestPoints(target: S2DistanceTarget<T>, options: Options<T>, results: MutableList<Result<T, D>>) {
        ...
        if (options.getMaxResult() == 1) {
            ...
            results.reverse()
            Assertions.assert { results.isSorted() }
        }
    }
    ...
    private fun findClosestPointsOptimized() {
        ...
        while (!queue.isEmpty()) {
            ...
            if (distance >= distanceLimit) {
                queue = ArrayDeque(16)  // Clear any remaining entries.
                break
            }
    ...
    private fun maybeAddResult(pointData: PointData<D>) {
        ...
        if (options.getMaxResult() == 1) {
            ...
            if (resultSet.size >= options.getMaxResult()) {
                resultSet.poll()  // Replace the furthest result point.
            }
    ...
    private fun processOrEnqueue(id: S2CellId, iter: S2PointIndexIterator<D>, seek: Boolean): Boolean {
