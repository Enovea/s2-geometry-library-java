...
import dilivia.s2.LookupCellTables.kLookupBits
import dilivia.s2.LookupCellTables.lookupIj
 ...
 * @constructor Creates a cell id with the specified ULong value
 * @param id The id of the cell as ULong.
 * @see S2Coords
 */
@ExperimentalUnsignedTypes
@Strictfp
class S2CellId(val id: ULong) : Comparable<S2CellId> {
     ...
     */
    fun toPointRaw(): S2Point = S2Coords.faceSiTitoXYZ(getCenterSiTi())
    ...
    /**
     * Get the center of the cell in (s,t) coordinates.
     *
    ...
    /**
     * Get the edge length of this cell in (s,t)-space.
     *
     * @return The size of the cell un (s,t)-space. In the range [0, 1]
     */
    fun getSizeST(): Double = getSizeST(level())

    /**
     * Get the bounds of this cell in (s,t)-space.
     *
     * @return The bounds of this cell.
     */
    fun getBoundST(): R2Rect {
        val size = getSizeST()
        return R2Rect.fromCenterSize(getCenterST(), R2Point(size, size))
    }

    /**
     * Get the center of the cell in (u,v) coordinates.
    ...
    */
    ...
    /**
     * Get the bound of this cell in (u,v)-space.
     *
     ...
     * @return true is the cell is valid.
     */
    fun isValid(): Boolean = (face() < kNumFaces && (lsb() and 0x1555555555555555UL) != 0UL)
    ...
    /**
     * Get the edge length of this cell in (i,j)-space.
     *
     ...
     */
    fun isLeaf(): Boolean = id.toInt() and 1 != 0

    /**
     * Indicate if this is a top-level face cell (more efficient than checking whether level() == 0).
     *
     * @return true if the cell is a face.
     */
    fun isFace(): Boolean = (id and (lsbForLevel(0) - 1UL)) == 0UL
     ...
     */
    fun childPosition(): Int = childPosition(level())
    ...
    */
    ...
    fun childBegin(level: Int): S2CellId {
        assert { isValid() }
        assertGE(level, this.level())
        assertLE(level, kMaxLevel)
     ...
     */
    fun previous(): S2CellId {
        return S2CellId(id - (lsb() shl 1))
    }
    ...
    */
    ...
    internal fun getCommonAncestorLevel(other: S2CellId): Int {
        ...
        return max(60 - Bits.findMSBSetNonZero64(bits), -1) shr 1
    }
    ...
    */
    ...
    fun lessOrEquals(other: S2CellId): Boolean = id <= other.id
    fun greaterOrEquals(other: S2CellId): Boolean = id >= other.id
    fun greaterThan(other: S2CellId): Boolean = id > other.id
    fun lessThan(other: S2CellId): Boolean = id < other.id
    ...
    companion object {
        ...
        @JvmStatic
        fun hexFormatString(value: ULong, num_digits: Int): String {
            var result = ""
            var i = num_digits
            var v = value
            while (i >= 1) {
