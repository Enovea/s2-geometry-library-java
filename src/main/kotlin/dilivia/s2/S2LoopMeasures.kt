...
object S2LoopMeasures {
  ...
  fun getPerimeter(loop: S2PointLoopSpan): S1Angle {
    val perimeter = S1Angle.zero.toMutable()
  ...
  internal fun pruneDegeneracies(loop: S2PointLoopSpan): S2PointLoopSpan {
    ...
    return if (spanLength < 3) S2PointLoopSpan(emptyList()) else S2PointLoopSpan(vertices.subList(k, k + spanLength))
  }

  private val kMaxCurvature = 2 * M_PI - 4 * DBL_EPSILON
  ...
  private val kMaxErrorPerVertex = 9.73 * DBL_EPSILON
  ...
  fun getCentroid(loop: S2PointLoopSpan): S2Point {
    ...
    return getSurfaceIntegral(loop, SummableS2Point(S2Point())) { a: S2Point, b: S2Point, c: S2Point -> SummableS2Point(S2Centroids.trueCentroid(a, b, c)) }
  }
  ...
  fun <T, V> getSurfaceIntegral(loop: S2PointLoopSpan, zero: T, f_tri: (S2Point, S2Point, S2Point) -> T): V where T: Summable<V> {
    ...
    while (i + 1 < loop.size) {
      ...
      if (loop[i + 1].angle(origin) > kMaxLength) {
        ...
        if (origin == loop[0]) {
          ...
          origin = S2Point.robustCrossProd(loop[0], loop[i]).normalize()
