...
public strictfp class S2LatLngRect implements S2Region {
  ...
  public boolean isValid() {
    ...
    return (Math.abs(lat.getLo()) <= S2.M_PI_2 && Math.abs(lat.getHi()) <= S2.M_PI_2
      && lng.isValid() && lat.isEmpty() == lng.isEmpty());
  }

  // Accessor methods.
  public S1Angle latLo() {
    return S1Angle.radians(lat.getLo());
  }

  public S1Angle latHi() {
    return S1Angle.radians(lat.getHi());
  }
  ...
  public S1Angle getDistance(S2LatLng p) {
    ...
    S2Point hi = S2LatLng.fromRadians(a.lat().getHi(), aLng).toPoint();
    S2Point loCrossHi =
  ...
  public boolean intersects(S2Cell cell) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (intersectsLatEdge(a, b, lat.getLo(), lng)) {
        return true;
      }
      if (intersectsLatEdge(a, b, lat.getHi(), lng)) {
        return true;
      }
    }
    return false;
  ...
  private static boolean intersectsLngEdge(S2Point a, S2Point b,
    ...
    return S2.simpleCrossing(a, b, S2LatLng.fromRadians(lat.getLo(), lng)
      .toPoint(), S2LatLng.fromRadians(lat.getHi(), lng).toPoint());
  }
