...
public final strictfp class S2Cell implements S2Region {
    ...
    private static final int MAX_CELL_SIZE = 1 << S2CellId.kMaxLevel;
    ...
    public boolean isLeaf() {
        return level == S2CellId.kMaxLevel;
    }
    ...
    public R2Vector getCenterUV() {
        FaceIJ faceIJ = cellId.toFaceIJOrientation();
        int cellSize = 1 << (S2CellId.kMaxLevel - level);

        // TODO(dbeaumont): Figure out a better naming of the variables here (and elsewhere).
        int si = (faceIJ.getI() & -cellSize) * 2 + cellSize - MAX_CELL_SIZE;
        double x = S2Projections.stToUV((1.0 / MAX_CELL_SIZE) * si);

        int sj = (faceIJ.getJ() & -cellSize) * 2 + cellSize - MAX_CELL_SIZE;
    ...
    private void init(S2CellId id) {
        ...
        FaceIJ faceIJ = id.toFaceIJOrientation(true);
        face = (byte) faceIJ.getFace();
        //noinspection ConstantConditions
        orientation = faceIJ.getOrientation().byteValue(); // Compress int to a byte.
        level = (byte) id.level();
        int cellSize = 1 << (S2CellId.kMaxLevel - level);
        int[] ij = new int[]{faceIJ.getI(), faceIJ.getJ()};
        for (int d = 0; d < 2; ++d) {
